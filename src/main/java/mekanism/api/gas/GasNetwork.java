 ...
 * ticked on
 * the client side and if it's posted events are handled properly, it has the capability to visually display gasses
 * network-wide.
 * 
 * @author aidancbrady
 *
...
public class GasNetwork extends DynamicNetwork<IGasHandler, GasNetwork> {
    ...
    public int tickEmit(GasStack stack) {
        ...
        if (sent > 0 && FMLCommonHandler.instance()
            .getEffectiveSide()
            .isServer()) {
            didTransfer = true;
            transferDelay = 2;
    ...
    public String getStoredInfo() {
        return buffer != null ? buffer.getGas()
            .getLocalizedName() + " ("
            + buffer.amount
            + ")" : "None";
    }
