...
public class ItemMeta extends Item {
    ...
    public void registerIcons(IIconRegister reg) {
        ...
        for (int j = 0; j < subItems % 6; j++) {
            icons[(subItems - (subItems % 6)) / 6][j] = reg
                .registerIcon("nc:" + folders + (folders == "" ? "" : "/") + (j + subItems - (subItems % 6)));
        }
    }
    ...
    public IIcon getIconFromDamage(int meta) {
        ...
        if (meta >= subItems - (subItems % 6) && meta < subItems)
            return this.icons[(subItems - (subItems % 6)) / 6][meta - (subItems - (subItems % 6))];
