...
public class Matrix {
    ...
    public static Complex[][] spinW(double s, double t, double p) {
        return add(
            multiply(Math.cos(t * Math.PI / 180), spinZ(s)),
            add(
                multiply(Math.sin(t * Math.PI / 180) * Math.cos(p * Math.PI / 180), spinX(s)),
                multiply(Math.sin(t * Math.PI / 180) * Math.sin(p * Math.PI / 180), spinY(s))));
    }
    ...
    public static Complex[][] twoSpinInteraction(int a, int b, double... spin) {
        return add(
            multiply(spinZ(a, spin), spinZ(b, spin)),
            add(multiply(spinX(a, spin), spinX(b, spin)), multiply(spinY(a, spin), spinY(b, spin))));
    }
