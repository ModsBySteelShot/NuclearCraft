...
public class ContainerNuclearWorkspace extends Container {
    ...
    public boolean canInteractWith(EntityPlayer player) {
        return this.worldObj.getBlock(this.posX, this.posY, this.posZ) != NCBlocks.nuclearWorkspace ? false
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slotNumber) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotNumber == 0) {
                ...
                if (NuclearCraft.workspaceShiftClick
                    ? (!this.mergeItemStack(itemstack1, 1, 26, false)
                        && !this.mergeItemStack(itemstack1, 53, 62, false))
                    : !this.mergeItemStack(itemstack1, 53, 62, false)) {
                    return null;
                }
