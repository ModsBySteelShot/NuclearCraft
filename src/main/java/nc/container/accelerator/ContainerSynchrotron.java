...
public class ContainerSynchrotron extends Container {
    ...
    public void updateProgressBar(int slot, int value) {
        ...
        if (slot == 2) {
            this.lastLength = this.upcastShort(value);
        }
        if (slot == 3) {
            this.lastEfficiency = this.upcastShort(value);
        }
        if (slot == 4) {
            this.lastFuel = this.upcastShort(value);
        }
        if (slot == 5) {
            this.lastParticleEnergy = this.upcastShort(value);
        }
        if (slot == 6) {
            this.lastPercentageOn = this.upcastShort(value);
        }
        ...
        if (slot == 8) {
            this.entity.length = this.lastLength | value << 16;
        }
        if (slot == 9) {
            this.entity.efficiency = this.lastEfficiency | value << 16;
        }
        if (slot == 10) {
            this.entity.fuel = this.lastFuel | value << 16;
        }
        if (slot == 11) {
            this.entity.particleEnergy = this.lastParticleEnergy | value << 16;
        }
        if (slot == 12) {
            this.entity.percentageOn = this.lastPercentageOn | value << 16;
        }

        if (slot == 13) {
            this.lastRadiationPower = this.upcastShort(value);
        }
        if (slot == 14) {
            this.entity.radiationPower = this.lastRadiationPower | value << 16;
        }
        ...
        if (slot == 17) {
            this.lastAntimatter = this.upcastShort(value);
        }
        if (slot == 18) {
            this.entity.antimatter = this.lastAntimatter | value << 16;
        }
    }
