...
public class ContainerFusionReactorSteam extends Container {
    ...
    public void updateProgressBar(int slot, int value) {
        ...
        if (slot == 2) {
            this.lastHLevel = this.upcastShort(value);
        }
        if (slot == 3) {
            this.lastDLevel = this.upcastShort(value);
        }
        if (slot == 4) {
            this.lastTLevel = this.upcastShort(value);
        }
        if (slot == 5) {
            this.lastHeLevel = this.upcastShort(value);
        }
        if (slot == 6) {
            this.lastBLevel = this.upcastShort(value);
        }
        if (slot == 7) {
            this.lastLi6Level = this.upcastShort(value);
        }
        if (slot == 8) {
            this.lastLi7Level = this.upcastShort(value);
        }
        if (slot == 9) {
            this.lastHLevel2 = this.upcastShort(value);
        }
        if (slot == 10) {
            this.lastDLevel2 = this.upcastShort(value);
        }
        if (slot == 11) {
            this.lastTLevel2 = this.upcastShort(value);
        }
        if (slot == 12) {
            this.lastHeLevel2 = this.upcastShort(value);
        }
        if (slot == 13) {
            this.lastBLevel2 = this.upcastShort(value);
        }
        if (slot == 14) {
            this.lastLi6Level2 = this.upcastShort(value);
        }
        if (slot == 15) {
            this.lastLi7Level2 = this.upcastShort(value);
        }
        ...
        if (slot == 17) {
            this.entity.HLevel = this.lastHLevel | value << 16;
        }
        if (slot == 18) {
            this.entity.DLevel = this.lastDLevel | value << 16;
        }
        if (slot == 19) {
            this.entity.TLevel = this.lastTLevel | value << 16;
        }
        if (slot == 20) {
            this.entity.HeLevel = this.lastHeLevel | value << 16;
        }
        if (slot == 21) {
            this.entity.BLevel = this.lastBLevel | value << 16;
        }
        if (slot == 22) {
            this.entity.Li6Level = this.lastLi6Level | value << 16;
        }
        if (slot == 23) {
            this.entity.Li7Level = this.lastLi7Level | value << 16;
        }
        if (slot == 24) {
            this.entity.HLevel2 = this.lastHLevel2 | value << 16;
        }
        if (slot == 25) {
            this.entity.DLevel2 = this.lastDLevel2 | value << 16;
        }
        if (slot == 26) {
            this.entity.TLevel2 = this.lastTLevel2 | value << 16;
        }
        if (slot == 27) {
            this.entity.HeLevel2 = this.lastHeLevel2 | value << 16;
        }
        if (slot == 28) {
            this.entity.BLevel2 = this.lastBLevel2 | value << 16;
        }
        if (slot == 29) {
            this.entity.Li6Level2 = this.lastLi6Level2 | value << 16;
        }
        if (slot == 30) {
            this.entity.Li7Level2 = this.lastLi7Level2 | value << 16;
        }

        if (slot == 31) {
            this.lastSShown = this.upcastShort(value);
        }
        if (slot == 32) {
            this.entity.SShown = this.lastSShown | value << 16;
        }
        if (slot == 33) {
            this.lastSize = this.upcastShort(value);
        }
        if (slot == 34) {
            this.entity.size = this.lastSize | value << 16;
        }
        if (slot == 35) {
            this.lastBelow = this.upcastShort(value);
        }

        if (slot == 37) {
            this.lastEfficiency = this.upcastShort(value);
        }
        if (slot == 38) {
            this.entity.efficiency = (int) this.lastEfficiency | value << 16;
        }
        if (slot == 39) {
            this.lastHeat = this.upcastShort(value);
        }
        if (slot == 40) {
            this.entity.heat = (int) this.lastHeat | value << 16;
        }
        if (slot == 41) {
            this.lastHeatVar = this.upcastShort(value);
        }
        if (slot == 42) {
            this.entity.heatVar = (int) this.lastHeatVar | value << 16;
        }

        if (slot == 43) {
            this.lastHOut = this.upcastShort(value);
        }
        if (slot == 44) {
            this.entity.HOut = (int) this.lastHOut | value << 16;
        }
        if (slot == 45) {
            this.lastDOut = this.upcastShort(value);
        }
        if (slot == 46) {
            this.entity.DOut = (int) this.lastDOut | value << 16;
        }
        if (slot == 47) {
            this.lastTOut = this.upcastShort(value);
        }
        if (slot == 48) {
            this.entity.TOut = (int) this.lastTOut | value << 16;
        }
        if (slot == 49) {
            this.lastHE3Out = this.upcastShort(value);
        }
        if (slot == 50) {
            this.entity.HE3Out = (int) this.lastHE3Out | value << 16;
        }
        if (slot == 51) {
            this.lastHE4Out = this.upcastShort(value);
        }
        if (slot == 52) {
            this.entity.HE4Out = (int) this.lastHE4Out | value << 16;
        }
        if (slot == 53) {
            this.lastnOut = this.upcastShort(value);
        }
        if (slot == 54) {
            this.entity.nOut = (int) this.lastnOut | value << 16;
        }

        if (slot == 55) {
            this.lastComplete = this.upcastShort(value);
        }
        if (slot == 56) {
            this.entity.complete = this.lastComplete | value << 16;
        }
        ...
        if (slot == 59) {
            this.lastSteam = this.upcastShort(value);
        }
        if (slot == 60) {
            this.entity.steam = (int) this.lastSteam | value << 16;
        }

        if (slot == 61) {
            this.lastSteamType = this.upcastShort(value);
        }
        if (slot == 62) {
            this.entity.steamType = this.lastSteamType | value << 16;
        }
    }
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int clickedSlotNumber) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (clickedSlotNumber > 8) {
                ...
                } else if (clickedSlotNumber >= 36 && clickedSlotNumber < 45
