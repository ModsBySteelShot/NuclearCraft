...
public class ContainerFissionReactor extends Container {
    ...
    public void updateProgressBar(int slot, int value) {
        ...
        if (slot == 3) {
            this.lastFueltime = this.upcastShort(value);
        }
        ...
        if (slot == 5) {
            this.entity.heat = this.lastheat | value << 16;
        }
        if (slot == 6) {
            this.entity.fueltime = this.lastFueltime | value << 16;
        }

        if (slot == 7) {
            this.lastE = this.upcastShort(value);
        }
        if (slot == 8) {
            this.entity.E = this.lastE | value << 16;
        }
        if (slot == 9) {
            this.lastH = this.upcastShort(value);
        }
        if (slot == 10) {
            this.entity.H = this.lastH | value << 16;
        }

        if (slot == 11) {
            this.lastOff = this.upcastShort(value);
        }
        if (slot == 12) {
            this.entity.off = this.lastOff | value << 16;
        }

        if (slot == 13) {
            this.lastFuelType = this.upcastShort(value);
        }
        if (slot == 14) {
            this.entity.fueltype = this.lastFuelType | value << 16;
        }

        if (slot == 15) {
            this.lastEReal = this.upcastShort(value);
        }
        if (slot == 16) {
            this.entity.EReal = this.lastEReal | value << 16;
        }
        if (slot == 17) {
            this.lastHReal = this.upcastShort(value);
        }
        if (slot == 18) {
            this.entity.HReal = this.lastHReal | value << 16;
        }
        if (slot == 19) {
            this.lastFReal = this.upcastShort(value);
        }
        if (slot == 20) {
            this.entity.FReal = this.lastFReal | value << 16;
        }
        ...
        if (slot == 23) {
            this.lastlx = this.upcastShort(value);
        }
        if (slot == 24) {
            this.entity.lx = this.lastlx | value << 16;
        }
        if (slot == 25) {
            this.lastly = this.upcastShort(value);
        }
        if (slot == 26) {
            this.entity.ly = this.lastly | value << 16;
        }
        if (slot == 27) {
            this.lastlz = this.upcastShort(value);
        }
        if (slot == 28) {
            this.entity.lz = this.lastlz | value << 16;
        }
        ...
        if (slot == 30) {
            this.entity.efficiency = this.lastEfficiency | value << 16;
        }

        if (slot == 31) {
            this.lastNumberOfCells = this.upcastShort(value);
        }
        if (slot == 32) {
            this.entity.numberOfCells = this.lastNumberOfCells | value << 16;
        }

        if (slot == 33) {
            this.lastHCooling = this.upcastShort(value);
        }
        if (slot == 34) {
            this.entity.HCooling = this.lastHCooling | value << 16;
        }

        if (slot == 35) {
            this.lastMinusH = this.upcastShort(value);
        }
        if (slot == 36) {
            this.entity.MinusH = this.lastMinusH | value << 16;
        }
    }
