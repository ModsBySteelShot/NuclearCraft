...
public class GuiSynchrotron extends GuiContainer {
    ...
    public void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        String name1 = (this.entity.length + 2) + "x"
            + (this.entity.length + 2)
            + " "
            + StatCollector.translateToLocal("gui.synchrotron");
        ...
        String efficiency = StatCollector.translateToLocal("gui.efficiency") + ": "
            + Math.round(this.entity.efficiency * 0.0001)
            + "%";
        this.fontRendererObj.drawString(efficiency, 47, 38, (this.entity.complete == 1 ? -1 : 15597568));
        String particleEnergy = this.entity.particleEnergy * eff < 10000
            ? (this.entity.particleEnergy * eff < 10
                ? StatCollector.translateToLocal("gui.electronEnergy") + ": "
                    + Math.round(this.entity.particleEnergy * eff)
                    + " keV"
                : StatCollector.translateToLocal("gui.electronEnergy") + ": "
                    + Math.round(this.entity.particleEnergy * eff)
                    + " MeV")
            : StatCollector.translateToLocal("gui.electronEnergy") + ": "
                + Math.round(this.entity.particleEnergy * eff / 1000)
                + " GeV";
        this.fontRendererObj.drawString(particleEnergy, 47, 49, (this.entity.complete == 1 ? -1 : 15597568));
        String percentageOn = StatCollector.translateToLocal("gui.percentageOn") + ": "
            + Math.round(this.entity.percentageOn)
            + "%";
        this.fontRendererObj.drawString(percentageOn, 47, 60, (this.entity.complete == 1 ? -1 : 15597568));

        String rgen = this.entity.radiationPower < 10000
            ? StatCollector.translateToLocal("gui.radiationPower") + ": "
                + Math.round(this.entity.radiationPower)
                + " kW"
            : (this.entity.radiationPower > 10000000
                ? StatCollector.translateToLocal("gui.radiationPower") + ": "
                    + ((int) (Math.round(this.entity.radiationPower / 1000000)))
                    + " GW"
                : StatCollector.translateToLocal("gui.radiationPower") + ": "
                    + ((int) (Math.round(this.entity.radiationPower / 1000)))
                    + " MW");
        this.fontRendererObj.drawString(rgen, 47, 71, (this.entity.complete == 1 ? -1 : 15597568));

        String anti = StatCollector.translateToLocal("gui.antimatterProgress") + ": "
            + (int) ((this.entity.antimatter) / 64)
            + " ppm";
    ...
    protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) {
        ...
        this.drawTexturedModalRect(
            ...
            this.guiTop + 6 + 109 - k,
            176,
            3 + 109 - k + (this.entity.complete == 1 ? 0 : 109),
            5,
            k);
        int k2 = (int) (this.entity.fuel * 109 / 100000);
        this.drawTexturedModalRect(
            this.guiLeft + 18,
            this.guiTop + 6 + 109 - k2,
            181,
            3 + 109 - k2 + (this.entity.complete == 1 ? 0 : 109),
            5,
            k2);
        int k3 = (int) (this.entity.efficiency / 1000 * 109 / 1000);
        this.drawTexturedModalRect(
            this.guiLeft + 28,
            this.guiTop + 6 + 109 - k3,
            186,
            3 + 109 - k3 + (this.entity.complete == 1 ? 0 : 109),
            5,
            k3);
        int k4 = (int) (this.entity.percentageOn * 109 / 100);
        this.drawTexturedModalRect(
            this.guiLeft + 38,
            this.guiTop + 6 + 109 - k4,
            191,
            3 + 109 - k4 + (this.entity.complete == 1 ? 0 : 109),
            5,
            k4);

        int antimatterGUI = (int) Math.round(this.entity.antimatter * 14 / 64000000);
        if (this.entity.antimatter <= 64000000)
            drawTexturedModalRect(guiLeft + 153, guiTop + 117, 176, 2, antimatterGUI, 1);
        else drawTexturedModalRect(guiLeft + 153, guiTop + 117, 176, 2, 14, 1);
    }
}
