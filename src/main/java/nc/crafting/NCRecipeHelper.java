...
public abstract class NCRecipeHelper {
    ...
    public void addRecipe(Object... objects) {
        ...
        for (int i = 0; i < objects.length; i++) {
            ...
            if (objects[i] instanceof String) {
                if (i < inputSize) {
                    if (OreDictionary.getOres(((String) objects[i]))
                        .size() > 0) {
                ...
                if (i < inputSize) {
                    if (OreDictionary.getOres(((OreStack) objects[i]).oreString)
                        .size() > 0) {
                        stack[i] = objects[i];
                    } else {
                        return;
                    ...
                    if (ores.size() > 0) {
                        stack[i] = new ItemStack(
                            ores.get(0)
                                .getItem(),
                            ((OreStack) objects[i]).stackSize,
                            ores.get(0)
                                .getItemDamage());
                    } else {
                        return;
    ...
    private boolean equalStack(ItemStack stack, ItemStack key, boolean checkSize) {
        ...
        return stack.getItem() == key.getItem() && (stack.getItemDamage() == key.getItemDamage())
        ...
        for (int i = 0; i < object.length; i++) {
            if (object[i] instanceof ItemStack) {
                ...
                for (int o = 0; o < ore.size(); o++) {
                    ores[o] = new ItemStack(
                        ore.get(o)
                            .getItem(),
                        ((OreStack) object[i]).stackSize,
                        ore.get(o)
                            .getItemDamage());
                }
                stack[i] = ores;
