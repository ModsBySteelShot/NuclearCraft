...
public abstract class ItemBlockEnergyStorage extends ItemBlockNC implements IEnergyContainerItem {
    ...
    public ItemBlockEnergyStorage(Block block, int storage) {
        super(
            block,
            "Stores "
                + (storage >= 10000000 ? storage / 1000000 + " M"
                    : (storage >= 10000 ? storage / 1000 + " k" : storage + " "))
                + "RF. Right click on a side without sneaking to",
            "change the side mode to input, output or disabled.");
        storedRF = storage;
    }
    ...
    public void addInformation(ItemStack itemStack, EntityPlayer player, List list, boolean whatIsThis) {
        ...
        if (tagCompound != null && tagCompound.getInteger("Energy") > 0) {
            list.add(
                EnumChatFormatting.LIGHT_PURPLE + "Energy: " + getEnergyStored(itemStack) + " / " + storedRF + " RF");
        }
