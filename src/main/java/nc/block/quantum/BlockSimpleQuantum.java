...
public class BlockSimpleQuantum extends BlockContainer {
    ...
    public void registerBlockIcons(IIconRegister i) {
        blockIcon = i.registerIcon(
            "nc:" + this.getUnlocalizedName()
                .substring(5));
    }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (!player.isSneaking()) {
            ...
            // double displayedAngle = (player.rotationYaw - t.angle + 360D) % 360 <= 180 ? (player.rotationYaw -
            // t.angle + 360D) % 360 : 360 - ((player.rotationYaw - t.angle + 360D) % 360);
            ...
            // if (world.isRemote) player.addChatMessage(new ChatComponentText(EnumChatFormatting.WHITE + ("Angle: " +
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase elb, ItemStack itemstack) {
        ...
        // if (world.isRemote) ((ICommandSender) elb).addChatMessage(new ChatComponentText(EnumChatFormatting.WHITE +
