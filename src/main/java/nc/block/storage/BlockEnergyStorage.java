...
public abstract class BlockEnergyStorage extends BlockContainer {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (player != null) {
            if (world.getTileEntity(x, y, z) instanceof TileStorage) {
                ...
                if (world.isRemote) {
                    if (player.isSneaking()) player.addChatMessage(
                        new ChatComponentText(
                            EnumChatFormatting.WHITE + "Energy Stored: "
                                + te.storage.getEnergyStored()
                                + " / "
                                + te.storage.getMaxEnergyStored()
                                + " RF"));
                    else player.addChatMessage(
                        new ChatComponentText(
                            EnumChatFormatting.WHITE + "Mode: "
                                + (te.sideMode[side] == 0 ? "Output"
                                    : (te.sideMode[side] == 1 ? "Disabled" : "Input"))));
                }
    ...
    public final ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune) {
        return Lists.newArrayList(
            getDataDrop(world, x, y, z, world.getTileEntity(x, y, z) != null ? world.getTileEntity(x, y, z) : null));
    }
