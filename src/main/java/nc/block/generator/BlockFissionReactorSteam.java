...
public class BlockFissionReactorSteam extends BlockContainer {
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
        ...
        this.iconFront = iconRegister
            .registerIcon("nc:generator/fissionReactorSteam/" + "front" + (this.isActive ? "Active" : "Idle"));
    ...
    public IIcon getIcon(int side, int metadata) {
        return metadata == 0 && side == 3 ? this.iconFront
            : (side == metadata ? this.iconFront
                : (side == 0 ? this.iconBottom : (side == 1 ? this.iconTop : this.blockIcon)));
    }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (world.isRemote) {
            ...
            FMLNetworkHandler
                .openGui(player, NuclearCraft.instance, NuclearCraft.guiIdFissionReactorSteam, world, x, y, z);
        }
        return true;
    }
    ...
    public int getComparatorInputOverride(World world, int x, int y, int z, int i) {
        TileFissionReactorSteam tileentity = (TileFissionReactorSteam) world.getTileEntity(x, y, z);
        return tileentity.heat <= NuclearCraft.fissionComparatorHeat
            ? (int) (15D * tileentity.heat / NuclearCraft.fissionComparatorHeat)
            : 15;
    }
