...
public abstract class TileMachineFluidOut extends TileMachineBase implements IFluidHandler {
    ...
    public TileMachineFluidOut(String localName, int energyMax, int fluidMax, int inSize, int outSize,
        boolean usesUpgrades, boolean usesEnergy, double pt, double er, Fluid flType, int flAmount, int smod, int tmod,
        NCRecipeHelper ncrecipes) {
    ...
    public boolean canCook() {
        ...
        if (getEnergyRequired > this.energyStorage.getMaxEnergyStored() && cookTime <= 0
            && this.energyStorage.getEnergyStored()
                < this.energyStorage.getMaxEnergyStored() - (int) Math.ceil(getEnergyRequired / getProcessTime)) {
            flag = false;
            return false;
        }
        if (getEnergyRequired < this.energyStorage.getMaxEnergyStored() && cookTime <= 0
            && getEnergyRequired > this.energyStorage.getEnergyStored()) {
            flag = false;
            return false;
        }
        if (hasEnergy) {
            ...
            if (output[j] == null) {
                ...
                if (this.slots[j + inputSize] != null) {
                    if (!this.slots[j + inputSize].isItemEqual(output[j])) {
                        flag = false;
                        return false;
                    } else if (this.slots[j + inputSize].stackSize + output[j].stackSize
                        > this.slots[j + inputSize].getMaxStackSize()) {
                            flag = false;
                            return false;
                        }
                }
