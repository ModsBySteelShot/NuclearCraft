...
public class TileAssembler extends TileMachineBase {
    ...
    public TileAssembler() {
        super(
            ...
            100,
            2000,
            NuclearCraft.assemblerSpeed,
            NuclearCraft.assemblerEfficiency,
    ...
    public boolean canCook() {
        ...
        if (getEnergyRequired > energyStorage.getMaxEnergyStored() && cookTime <= 0
            && energyStorage.getEnergyStored()
                < energyStorage.getMaxEnergyStored() - (int) Math.ceil(getEnergyRequired / getProcessTime)) {
            flag = false;
            return false;
        }
        if (hasEnergy) {
            ...
            if (output[j] == null) {
                ...
                if (slots[j + inputSize] != null) {
                    if (!slots[j + inputSize].isItemEqual(output[j])) {
                        flag = false;
                        return false;
                    } else if (slots[j + inputSize].stackSize + output[j].stackSize
                        > slots[j + inputSize].getMaxStackSize()) {
                            flag = false;
                            return false;
                        }
                }
