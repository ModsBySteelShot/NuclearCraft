...
public class TileElectricFurnace extends TileInventory implements IEnergyHandler, IEnergyReceiver {
    ...
    public void updateEntity() {
        ...
        if (flag != flag1) {
            flag1 = flag;
            BlockElectricFurnace.updateBlockState(flag, worldObj, xCoord, yCoord, zCoord);
        }
        markDirty();
    }
    ...
    public boolean canCook() {
        ...
        if (getRequiredEnergy > energyStorage.getMaxEnergyStored() && cookTime <= 0
            && energyStorage.getEnergyStored()
                < energyStorage.getMaxEnergyStored() - (int) Math.ceil(getRequiredEnergy / getFurnaceSpeed)) {
            flag = false;
            return false;
        }
        if (getRequiredEnergy < energyStorage.getMaxEnergyStored() && cookTime <= 0
            && getRequiredEnergy > energyStorage.getEnergyStored()) {
            flag = false;
            return false;
        }
