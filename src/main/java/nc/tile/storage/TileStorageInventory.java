...
public abstract class TileStorageInventory extends TileInventory
    implements IEnergyHandler, IEnergyReceiver, IEnergyProvider, IEnergyConnection, IReconfigurableSides {
    ...
    public boolean decrSide(int side) {
        if (!this.getWorldObj().isRemote) {
            ...
            PacketHandler.INSTANCE.sendToAllAround(
                new PacketSideConfig(xCoord, yCoord, zCoord, side, sideMode[side]),
                new TargetPoint(this.worldObj.provider.dimensionId, xCoord, yCoord, zCoord, 32));
        }
        return false;
    }
