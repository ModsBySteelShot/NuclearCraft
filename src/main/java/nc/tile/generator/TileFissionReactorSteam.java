...
public class TileFissionReactorSteam extends TileSteamProducer {
    ...
    public static String[] cellTypes = { "LEU235Cell", "HEU235Cell", "LEP239Cell", "HEP239Cell", "MOX239Cell",
        "TBUCell", "LEU233Cell", "HEU233Cell", "LEP241Cell", "HEP241Cell", "MOX241Cell",

        "LEU235CellOxide", "HEU235CellOxide", "LEP239CellOxide", "HEP239CellOxide", "LEU233CellOxide",
        "HEU233CellOxide", "LEP241CellOxide", "HEP241CellOxide",

        "LEN236Cell", "HEN236Cell", "LEA242Cell", "HEA242Cell", "LEC243Cell", "HEC243Cell", "LEC245Cell", "HEC245Cell",
        "LEC247Cell", "HEC247Cell",

        "LEN236CellOxide", "HEN236CellOxide", "LEA242CellOxide", "HEA242CellOxide", "LEC243CellOxide",
        "HEC243CellOxide", "LEC245CellOxide", "HEC245CellOxide", "LEC247CellOxide", "HEC247CellOxide", "TBUCellOxide" };

    public static String[] fuelTypes = { "LEU", "HEU", "LEP", "HEP", "MOX", "TBU", "LEU", "HEU", "LEP", "HEP", "MOX",

        "LEU-Ox", "HEU-Ox", "LEP-Ox", "HEP-Ox", "LEU-Ox", "HEU-Ox", "LEP-Ox", "HEP-Ox",

        "LEN", "HEN", "LEA", "HEA", "LEC", "HEC", "LEC", "HEC", "LEC", "HEC",

        "LEN-Ox", "HEN-Ox", "LEA-Ox", "HEA-Ox", "LEC-Ox", "HEC-Ox", "LEC-Ox", "HEC-Ox", "LEC-Ox", "HEC-Ox", "TBU-Ox" };
    ...
    public void overheat() {
        if (heat >= 1000000) {
            ...
            if (NuclearCraft.nuclearMeltdowns) {
                if (getBlockMetadata() == 4) NCExplosion.createExplosion(
                    new EntityBomb(worldObj).setType(BombType.BOMB_STANDARD),
                    worldObj,
                    xCoord + ((x0 + x1) / 2),
                    yCoord + ((y0 + y1) / 2),
                    zCoord + ((z0 + z1) / 2),
                    0.01F * (lx + ly + lz) * NuclearCraft.explosionRadius,
                    0.01F * (lx + ly + lz) * NuclearCraft.explosionRadius,
                    true);
                else if (getBlockMetadata() == 2) NCExplosion.createExplosion(
                    new EntityBomb(worldObj).setType(BombType.BOMB_STANDARD),
                    worldObj,
                    xCoord - ((z0 + z1) / 2),
                    yCoord + ((y0 + y1) / 2),
                    ...
                    0.01F * (lx + ly + lz) * NuclearCraft.explosionRadius,
                    true);
                else if (getBlockMetadata() == 5) NCExplosion.createExplosion(
                    new EntityBomb(worldObj).setType(BombType.BOMB_STANDARD),
                    worldObj,
                    xCoord - ((x0 + x1) / 2),
                    yCoord + ((y0 + y1) / 2),
                    ...
                    0.01F * (lx + ly + lz) * NuclearCraft.explosionRadius,
                    true);
                else if (getBlockMetadata() == 3) NCExplosion.createExplosion(
                    new EntityBomb(worldObj).setType(BombType.BOMB_STANDARD),
                    worldObj,
                    xCoord + ((z0 + z1) / 2),
                    yCoord + ((y0 + y1) / 2),
    ...
    private void steam() {
        ...
        if (doesFuelMatchDepleted() && worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)
            && fueltime > 0
            && fueltype != 0
            && complete == 1) {
            off = 0;
            flag = true;
        } else if (doesFuelMatchDepleted() && !worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)
            && fueltime > 0
            && fueltype != 0
            && complete == 1) {
                off = 1;
        ...
        if (tickCount >= NuclearCraft.fissionUpdateRate) {
                    ...
                    for (int x = x0 + 1; x <= x1 - 1; x++) {
                        for (int y = y0 + 1; y <= y1 - 1; y++) {
                            if (find(NCBlocks.cellBlock, x, y, z)) {
                                extraCells = 0;
                                if (find(NCBlocks.cellBlock, x + 1, y, z) || (find(NCBlocks.graphiteBlock, x + 1, y, z)
                                    && find(NCBlocks.cellBlock, x + 2, y, z))) extraCells += 1;
                                if (find(NCBlocks.cellBlock, x - 1, y, z) || (find(NCBlocks.graphiteBlock, x - 1, y, z)
                                    && find(NCBlocks.cellBlock, x - 2, y, z))) extraCells += 1;
                                if (find(NCBlocks.cellBlock, x, y + 1, z) || (find(NCBlocks.graphiteBlock, x, y + 1, z)
                                    && find(NCBlocks.cellBlock, x, y + 2, z))) extraCells += 1;
                                if (find(NCBlocks.cellBlock, x, y - 1, z) || (find(NCBlocks.graphiteBlock, x, y - 1, z)
                                    && find(NCBlocks.cellBlock, x, y - 2, z))) extraCells += 1;
                                if (find(NCBlocks.cellBlock, x, y, z + 1) || (find(NCBlocks.graphiteBlock, x, y, z + 1)
                                    && find(NCBlocks.cellBlock, x, y, z + 2))) extraCells += 1;
                                if (find(NCBlocks.cellBlock, x, y, z - 1) || (find(NCBlocks.graphiteBlock, x, y, z - 1)
                                    && find(NCBlocks.cellBlock, x, y, z - 2))) extraCells += 1;
            ...
            if (doesFuelMatchDepleted() && worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)
                ...
                steamThisTick += baseSteam * (10000 * sMult + heat)
                    * (numberOfCells + 2 * adj1 + 3 * adj2 + 4 * adj3 + 5 * adj4 + 6 * adj5 + 7 * adj6) /*
                                                                                                         * *Math.cbrt((
                                                                                                         * lx - 2)*(ly -
                                                                                                         * 2)*(lz - 2))
                                                                                                         */
                ...
                heatThisTick += baseHeat
                    * (numberOfCells + 3 * adj1 + 6 * adj2 + 10 * adj3 + 15 * adj4 + 21 * adj5 + 28 * adj6);
                fuelThisTick += (numberOfCells + adj1 + adj2 + adj3 + adj4 + adj5 + adj6) * baseFuel
                    / NuclearCraft.fissionEfficiency;

                for (int z = z0 + 1; z <= z1 - 1; z++) {
                    for (int x = x0 + 1; x <= x1 - 1; x++) {
                        for (int y = y0 + 1; y <= y1 - 1; y++) {
                            if (find(NCBlocks.graphiteBlock, x, y, z)) {
                                heatThisTick += (hMult / 100) * baseSteam
                                    * (numberOfCells + adj1 + adj2 + adj3 + adj4 + adj5 + adj6)
                                    / 5;
                                if (surroundOr(NCBlocks.cellBlock, x, y, z))
                                    steamThisTick += (10000 * sMult + heat) * baseSteam
                                        * (numberOfCells + adj1 + adj2 + adj3 + adj4 + adj5 + adj6)
                                        / 10000000;
                            }
                            if (find(NCBlocks.speedBlock, x, y, z)
                                && (numberOfCells + adj1 + adj2 + adj3 + adj4 + adj5 + adj6) > 0) {
                                if (lx - 2 + ly - 2 + lz - 2 > 0)
                                    fuelThisTick += (numberOfCells + adj1 + adj2 + adj3 + adj4 + adj5 + adj6) * baseFuel
                                        / (NuclearCraft.fissionEfficiency * (lx - 2 + ly - 2 + lz - 2));
                            }
                        }
                    }
                }
                ...
                && complete == 1) {
                    ...
                    fakeSteamThisTick += baseSteam * (10000 * sMult + heat)
                        * (numberOfCells + 2 * adj1 + 3 * adj2 + 4 * adj3 + 5 * adj4 + 6 * adj5 + 7 * adj6) /*
                                                                                                             * *Math.
                                                                                                             * cbrt((lx
                                                                                                             * - 2)*(ly
                                                                                                             ...
                                                                                                             */
                        / 1000000;
                    fakeHeatThisTick += baseHeat * (hMult / 100)
                        * (numberOfCells + 3 * adj1 + 6 * adj2 + 10 * adj3 + 15 * adj4 + 21 * adj5 + 28 * adj6);
                    ...
                    for (int z = z0 + 1; z <= z1 - 1; z++) {
                        for (int x = x0 + 1; x <= x1 - 1; x++) {
                            for (int y = y0 + 1; y <= y1 - 1; y++) {
                                if (find(NCBlocks.graphiteBlock, x, y, z)) {
                                    fakeHeatThisTick += (hMult / 100) * baseSteam
                                        * (numberOfCells + adj1 + adj2 + adj3 + adj4 + adj5 + adj6)
                                        / 5;
                                    if (surroundOr(NCBlocks.cellBlock, x, y, z))
                                        fakeSteamThisTick += (10000 * sMult + heat) * baseSteam
                                            * (numberOfCells + adj1 + adj2 + adj3 + adj4 + adj5 + adj6)
                                            / 10000000;
                                }
                            }
            ...
            if (complete == 1) efficiency = (int) (100 * (steamThisTick + fakeSteamThisTick)
                / (baseSteam * (sMult / 100)
                    * (numberOfCells + adj1
                        + adj2
                        + adj3
        ...
        if (off == 0 && flag) {
            ...
            if ((int) SReal < 200) {
                if (tank.getFluidAmount() != 0) if (tank.getFluid()
                    .getFluid() != NuclearCraft.steam) tank.drain(100000, true);
                if (steam >= 200) {
                ...
                if (tank.getFluidAmount() != 0) if (tank.getFluid()
                    .getFluid() != NuclearCraft.denseSteam) tank.drain(100000, true);
                if (steam >= 40000) {
                ...
                if (tank.getFluidAmount() != 0) if (tank.getFluid()
                    .getFluid() != NuclearCraft.superdenseSteam) tank.drain(100000, true);
                for (int i = 0; i < 100; i++) {
                    if (steam >= 1000000) {
    ...
    public boolean between(Block block, int x, int y, int z) {
        return ((findBasic(block, x + 1, y, z) && findBasic(block, x - 1, y, z))
            || (findBasic(block, x, y + 1, z) && findBasic(block, x, y - 1, z))
            || (findBasic(block, x, y, z + 1) && findBasic(block, x, y, z - 1)));
    }

    public boolean surroundOr(Block block, int x, int y, int z) {
        return (findBasic(block, x + 1, y, z) || findBasic(block, x - 1, y, z)
            || findBasic(block, x, y + 1, z)
            || findBasic(block, x, y - 1, z)
            || findBasic(block, x, y, z + 1)
            || findBasic(block, x, y, z - 1));
    }

    public boolean surroundOr(Block block, Block block2, int x, int y, int z) {
        return ((findBasic(block, x + 1, y, z) && findBasic(block2, x + 2, y, z))
            || (findBasic(block, x - 1, y, z) && findBasic(block2, x - 2, y, z))
            || (findBasic(block, x, y + 1, z) && findBasic(block2, x, y + 2, z))
            || (findBasic(block, x, y - 1, z) && findBasic(block2, x, y - 2, z))
            || (findBasic(block, x, y, z + 1) && findBasic(block2, x, y, z + 2))
            || (findBasic(block, x, y, z - 1) && findBasic(block2, x, y, z - 2)));
    }
        ...
        if (slots[1] != null) {
            ...
            if (OreDictionary.getOreIDs(slots[1]).length == 0 || OreDictionary.getOreIDs(slots[1]) == null)
                for (int id : OreDictionary.getOreIDs(slots[1])) {
                    if (OreDictionary.getOres("d" + cellTypes[type - 1])
                        .get(0)
                        .getItem()
                        == OreDictionary.getOres(OreDictionary.getOreName(id))
                            .get(0)
                            .getItem()
                        && OreDictionary.getOres("d" + cellTypes[type - 1])
                            .get(0)
                            .getItemDamage()
                            == OreDictionary.getOres(OreDictionary.getOreName(id))
                                .get(0)
                                .getItemDamage()
                        && slots[1].stackSize < 64) {
                        return true;
                    }
                }
            return false;
    ...
    private void product() {
        if (slots[1] == null && fueltime <= 0 && fueltype != 0) {
            for (int i = 0; i < cellTypes.length; i++) {
                if (fueltype == i + 1) {
                    slots[1] = new ItemStack(
                        OreDictionary.getOres("d" + cellTypes[i])
                            .get(0)
                            .getItem(),
                        1,
                        OreDictionary.getOres("d" + cellTypes[i])
            ...
            for (int i = 0; i < cellTypes.length; i++) {
                for (int id : OreDictionary.getOreIDs(slots[1])) {
                    if (OreDictionary.getOres("d" + cellTypes[i])
                        .get(0)
                        .getItem()
                        == OreDictionary.getOres(OreDictionary.getOreName(id))
                            .get(0)
    ...
    private boolean find(Block block, Block block2, Block block3, Block block4, int x, int y, int z) {
        ...
        if (getBlockMetadata() == 4)
            return (worldObj.getBlock(xc + x, yc, zc + z) == block || worldObj.getBlock(xc + x, yc, zc + z) == block2
                || worldObj.getBlock(xc + x, yc, zc + z) == block3
                || worldObj.getBlock(xc + x, yc, zc + z) == block4);
        else if (getBlockMetadata() == 2)
            return (worldObj.getBlock(xc - z, yc, zc + x) == block || worldObj.getBlock(xc - z, yc, zc + x) == block2
                || worldObj.getBlock(xc - z, yc, zc + x) == block3
                || worldObj.getBlock(xc - z, yc, zc + x) == block4);
    ...
    private boolean find(Block block, Block block2, Block block3, Block block4, Block block5, int x, int y, int z) {
        ...
        if (getBlockMetadata() == 4)
            return (worldObj.getBlock(xc + x, yc, zc + z) == block || worldObj.getBlock(xc + x, yc, zc + z) == block2
                || worldObj.getBlock(xc + x, yc, zc + z) == block3
                || worldObj.getBlock(xc + x, yc, zc + z) == block4
                || worldObj.getBlock(xc + x, yc, zc + z) == block5);
        else if (getBlockMetadata() == 2)
            return (worldObj.getBlock(xc - z, yc, zc + x) == block || worldObj.getBlock(xc - z, yc, zc + x) == block2
                || worldObj.getBlock(xc - z, yc, zc + x) == block3
                || worldObj.getBlock(xc - z, yc, zc + x) == block4
                || worldObj.getBlock(xc - z, yc, zc + x) == block5);
        else if (getBlockMetadata() == 5)
            return (worldObj.getBlock(xc - x, yc, zc - z) == block || worldObj.getBlock(xc - x, yc, zc - z) == block2
                || worldObj.getBlock(xc - x, yc, zc - z) == block3
                || worldObj.getBlock(xc - x, yc, zc - z) == block4
    ...
    private boolean checkStructure() {
        if (tickCount >= NuclearCraft.fissionUpdateRate) {
            ...
            int y1 = 0;
            for (int z = 0; z <= l; z++) {
                if ((find(b, 0, 1, 0) || find(b, 0, -1, 0))
                    || ((find(b, 1, 1, 0) || find(b, 1, -1, 0)) && find(b, 1, 0, 0))
                    || ((find(b, 1, 1, 0) && !find(b, 1, -1, 0)) && !find(b, 1, 0, 0))
                    || ((!find(b, 1, 1, 0) && find(b, 1, -1, 0)) && !find(b, 1, 0, 0))) {
                    if (/* !find(b, 0, 0, -z) && */ !find(b, 0, 1, -z) && !find(b, 0, -1, -z)
                        && (find(b, r, rr, rs, rrs, 0, 0, -z + 1) || find(b, r, rr, rs, rrs, 0, 1, -z + 1)
                            || find(b, r, rr, rs, rrs, 0, -1, -z + 1))) {
                        rz = l - z;
                        z0 = -z;
                        f = true;
                        break;
                    }
                } else if (!find(b, 0, 0, -z) && !find(b, 1, 1, -z)
                    && !find(b, 1, -1, -z)
                    && find(b, r, rr, rs, rrs, 0, 0, -z + 1)
                    && find(b, 1, 0, -z)
                    && find(b, 1, 1, -z + 1)
                    && find(b, 1, -1, -z + 1)) {
                        rz = l - z;
            ...
            f = false;
            for (int y = 0; y <= l; y++) {
                if (/* !find(b, x0, -y, z0) && */!find(b, x0, -y + 1, z0) && !find(b, x0 + 1, -y, z0)
                    && !find(b, x0, -y, z0 + 1)
                    && find(b, r, rr, rs, rrs, x0 + 1, -y, z0 + 1)
                    && find(b, r, rr, rs, rrs, x0, -y + 1, z0 + 1)
                    && find(b, r, rr, rs, rrs, x0 + 1, -y + 1, z0)) {
                    y0 = -y;
                    f = true;
                    break;
                }
            ...
            f = false;
            for (int z = 0; z <= rz; z++) {
                if (/* !find(b, x0, y0, z) && */ !find(b, x0, y0 + 1, z) && !find(b, x0 + 1, y0, z)
                    && !find(b, x0, y0, z - 1)
                    && find(b, r, rr, rs, rrs, x0 + 1, y0, z - 1)
                    && find(b, r, rr, rs, rrs, x0, y0 + 1, z - 1)
                    && find(b, r, rr, rs, rrs, x0 + 1, y0 + 1, z)) {
                    z1 = z;
                    f = true;
                    break;
                }
            ...
            f = false;
            for (int x = 0; x <= l; x++) {
                if (/* !find(b, x0 + x, y0, z0) && */ !find(b, x0 + x, y0 + 1, z0) && !find(b, x0 + x - 1, y0, z0)
                    && !find(b, x0 + x, y0, z0 + 1)
                    && find(b, r, rr, rs, rrs, x0 + x - 1, y0, z0 + 1)
                    && find(b, r, rr, rs, rrs, x0 + x, y0 + 1, z0 + 1)
                    && find(b, r, rr, rs, rrs, x0 + x - 1, y0 + 1, z0)) {
                    x1 = x0 + x;
                    f = true;
                    break;
                }
            ...
            f = false;
            for (int y = 0; y <= l; y++) {
                if (/* !find(b, x0, y0 + y, z0) && */ !find(b, x0, y0 + y - 1, z0) && !find(b, x0 + 1, y0 + y, z0)
                    && !find(b, x0, y0 + y, z0 + 1)
                    && find(b, r, rr, rs, rrs, x0 + 1, y0 + y, z0 + 1)
                    && find(b, r, rr, rs, rrs, x0, y0 + y - 1, z0 + 1)
                    && find(b, r, rr, rs, rrs, x0 + 1, y0 + y - 1, z0)) {
                    y1 = y0 + y;
                    f = true;
                    break;
                }
