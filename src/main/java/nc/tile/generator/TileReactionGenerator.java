...
public class TileReactionGenerator extends TileGeneratorInventory {
    ...
    public void updateEntity() {
        ...
        if (flag != flag1) {
            flag1 = flag;
            BlockReactionGenerator.updateBlockState(flag, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
        }
        markDirty();
    }
    ...
    private void energy() {
        ...
        if (this.fuellevel >= this.requiredFuel
            && this.storage.getEnergyStored() <= this.storage.getMaxEnergyStored() - TileReactionGenerator.power
    ...
    public static int fuelValue(ItemStack stack) {
        if (stack == null) return 0;
            ...
            else if (item == new ItemStack(NCItems.material, 1, 5).getItem() && item.getDamage(stack) == 5)
                return 10000;
            else if (item == new ItemStack(NCItems.material, 1, 19).getItem() && item.getDamage(stack) == 19)
                return 10000;
            else if (item == new ItemStack(NCItems.material, 1, 20).getItem() && item.getDamage(stack) == 20)
                return 10000;
            else if (item == new ItemStack(NCItems.material, 1, 53).getItem() && item.getDamage(stack) == 53)
                return 15000;
            else if (item == new ItemStack(NCItems.material, 1, 54).getItem() && item.getDamage(stack) == 54)
                return 15000;
            else if (item == new ItemStack(NCItems.material, 1, 126).getItem() && item.getDamage(stack) == 126)
                return 15000;
            else if (item == new ItemStack(NCItems.material, 1, 127).getItem() && item.getDamage(stack) == 127)
                return 15000;
