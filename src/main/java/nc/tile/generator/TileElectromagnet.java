...
public class TileElectromagnet extends TileInventory implements IEnergyHandler, IEnergyReceiver, ISidedInventory {
    ...
    public void updateEntity() {
        ...
        if (tickCount >= NuclearCraft.EMUpdateRate) {
            ...
            if (flag != flag1) {
                flag1 = flag;
                BlockElectromagnet.updateBlockState(flag, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
            }
            if (!flag1 && this.worldObj.getBlock(xCoord, yCoord, zCoord) == NCBlocks.electromagnetIdle
                && (pp(xCoord + 1, yCoord, zCoord) || pp(xCoord - 1, yCoord, zCoord)
                    || pp(xCoord, yCoord + 1, zCoord)
                    || pp(xCoord, yCoord - 1, zCoord)
                    || pp(xCoord, yCoord, zCoord + 1)
                    || pp(xCoord, yCoord, zCoord - 1))) {
                if (rand.nextFloat() > 1 - 0.005 * NuclearCraft.EMUpdateRate)
