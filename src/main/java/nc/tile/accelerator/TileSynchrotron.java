...
public class TileSynchrotron extends TileInventory implements IEnergyHandler, IEnergyReceiver, ISidedInventory {
    ...
    public void updateEntity() {
        ...
        if (flag != flag1) {
            flag1 = flag;
            BlockSynchrotron.updateBlockState(flag, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
        }

        if (soundCount >= 67) {
            if (storage.getEnergyStored() >= (int) (1000 * ((200 - (efficiency / 10000)) / 100))
                && worldObj.isBlockIndirectlyGettingPowered(this.xCoord, this.yCoord, this.zCoord)
                && flag1
                && percentageOn > 0
                && fuel - length >= 0
                && NuclearCraft.acceleratorSounds) {
    ...
    private void fuel() {
        ...
        if (stack != null && isFuel(stack)
            && fuel + 10000 <= fuelMax
            && (stack1 == null || (stack1 != null && stack1.stackSize < 64))) {
            fuel += 10000;
    ...
    private void power() {
        if (storage.getEnergyStored() >= (int) (1000 * ((200 - (efficiency / 10000)) / 100))
            ...
            && fuel - length >= 0) {
            fuel = fuel
                - (fuel < fuelMax / 2 ? length * ((fuel * 2) + 7500) / fuelMax : length * (fuelMax + 7500) / fuelMax)
                    / 4;
            this.storage.receiveEnergy(-(int) (1000 * ((200 - (efficiency / 10000)) / 100)), false);
            if (efficiency >= 1000000) efficiency = 1000000;
            else efficiency = efficiency + 5000 / (length + 1);
            if (length > 0) radiationPower = (fuel < fuelMax / 2 ? (fuel * 2) / fuelMax : 1) * (2 * 16
                * (((length * 100) + 1) / 100)
                * (((length * 100) + 1) / 100)
                * percentageOn
                * (NuclearCraft.superElectromagnetRF / 100)
                * (efficiency / 10000)) / 1000; // 20 kWatts per RF/t
            particleEnergy = (percentageOn / 100)
                * (m * c
                    * c
                    * (1 / Math.sqrt(
                        1 - Math.pow(
                                    ...
                                    * Math.pow(10, 5.5)
                                    - 3 * c
                                        ...
                                        * (2 * 16
                                            ...
                                            * (efficiency / 10000))
                                        * Math.pow(10, 10))
                                    / (8 * e * e * c * k * Math.pow(10, -15) - 3 * (length * 100)
                                        * (length * 100)
                                        * (2 * 16
                                            * (((length * 100) + 1) / 100)
                                            * (((length * 100) + 1) / 100)
            ...
            if (efficiency > 2500) efficiency = efficiency - 2500;
            else efficiency = 0;
            radiationPower = 0;
            particleEnergy = 0;
        }
     ...
     * if (storage.getEnergyStored() >= (int) (1000*((200-efficiency1)/100)) &&
     * worldObj.isBlockIndirectlyGettingPowered(this.xCoord, this.yCoord, this.zCoord) && flag && percentageOn > 0 &&
     * fuel - length >= 0) {
    ...
    private boolean tubef(int x, int y, int z) {
        ...
        return find(c, cc, x, y - 1, z - 1) && find(c, cc, x, y + 1, z - 1)
            && find(c, cc, x, y - 1, z + 1)
            && find(c, cc, x, y + 1, z + 1)
            && find(e, ee, x, y - 1, z)
            && find(e, ee, x, y, z + 1)
    ...
    private boolean tuber(int x, int y, int z) {
        ...
        Block c = NCBlocks.supercoolerIdle;
        Block cc = NCBlocks.supercoolerActive;
        return find(c, cc, x + 1, y - 1, z) && find(c, cc, x - 1, y - 1, z)
            && find(c, cc, x + 1, y + 1, z)
            && find(c, cc, x - 1, y + 1, z)
            && find(e, ee, x, y - 1, z)
            && find(e, ee, x + 1, y, z)
    ...
    private boolean tubefOn(int x, int y, int z) {
        ...
        Block ee = NCBlocks.superElectromagnetActive;
        Block cc = NCBlocks.supercoolerActive;
        return find(cc, x, y - 1, z - 1) && find(cc, x, y + 1, z - 1)
            && find(cc, x, y - 1, z + 1)
            && find(cc, x, y + 1, z + 1)
            && find(ee, x, y - 1, z)
            && find(ee, x, y, z + 1)
    ...
    private boolean tuberOn(int x, int y, int z) {
        ...
        Block ee = NCBlocks.superElectromagnetActive;
        Block cc = NCBlocks.supercoolerActive;
        return find(cc, x + 1, y - 1, z) && find(cc, x - 1, y - 1, z)
            && find(cc, x + 1, y + 1, z)
            && find(cc, x - 1, y + 1, z)
            && find(ee, x, y - 1, z)
            && find(ee, x + 1, y, z)
    ...
    private boolean cornertl(int x, int y, int z) {
        ...
        Block c = NCBlocks.supercoolerIdle;
        Block cc = NCBlocks.supercoolerActive;
        return find(c, cc, x - 1, y - 1, z - 1) && find(c, cc, x, y - 1, z - 1)
            && find(c, cc, x + 1, y - 1, z - 1)
            && find(c, cc, x + 1, y - 1, z)
            && find(c, cc, x + 1, y - 1, z + 1)
            && find(c, cc, x - 1, y - 1, z + 1)
            && find(e, ee, x - 1, y - 1, z)
    ...
    private boolean cornertlOn(int x, int y, int z) {
        ...
        Block ee = NCBlocks.superElectromagnetActive;
        Block cc = NCBlocks.supercoolerActive;
        return find(cc, x - 1, y - 1, z - 1) && find(cc, x, y - 1, z - 1)
            && find(cc, x + 1, y - 1, z - 1)
            && find(cc, x + 1, y - 1, z)
            && find(cc, x + 1, y - 1, z + 1)
            && find(cc, x - 1, y - 1, z + 1)
    ...
    private boolean checkRing() {
        ...
        if (cornerbl(0, 0, 0)) {
            ...
            l = lengthl();
            if (l < 1) {
                this.problem = StatCollector.translateToLocal("gui.ringNotBigEnough");
                flag = false;
                complete = 0;
                return false;
            }
            if (l > 2) for (int i = 0; i < l - 2; i++) {
                if (!tubef(2 + i, 0, 0) || !tuber(l + 1, 0, 2 + i) || !tubef(2 + i, 0, l + 1) || !tuber(0, 0, 2 + i)) {
                    this.problem = StatCollector.translateToLocal("gui.tubeIncomplete");
                    flag = false;
                    complete = 0;
                    return false;
                }
            ...
            if (!cornerbl(0, 0, 0) || !cornertl(1 + l, 0, 0) || !cornertr(1 + l, 0, 1 + l) || !cornerbr(0, 0, 1 + l)) {
                this.problem = StatCollector.translateToLocal("gui.cornerIncomplete");
                flag = false;
                complete = 0;
                return false;
            }
            if (find(s, ss, 0, 0, -2) || find(s, ss, l + 1, 0, -2)
                || find(s, ss, l + 3, 0, 0)
                || find(s, ss, l + 3, 0, l + 1)
                || find(s, ss, l + 1, 0, l + 3)
                || find(s, ss, 0, 0, l + 3)
            ...
            if (l > 2) for (int i = 0; i < l - 2; i++) {
                if (!tubef(2 + i, 0, 0) || !tuber(l + 1, 0, -(2 + i))
                    || !tubef(2 + i, 0, -(l + 1))
                    || !tuber(0, 0, -(2 + i))) {
                    this.problem = StatCollector.translateToLocal("gui.tubeIncomplete");
                    flag = false;
            ...
            if (!cornerbr(0, 0, 0) || !cornertr(1 + l, 0, 0)
                || !cornertl(1 + l, 0, -(1 + l))
                || !cornerbl(0, 0, -(1 + l))) {
                this.problem = StatCollector.translateToLocal("gui.cornerIncomplete");
                flag = false;
            ...
            this.problem = StatCollector.translateToLocal("gui.ringIncomplete");
            flag = false;
            complete = 0;
            return false;
        }
    ...
    public boolean percentageOn() {
        ...
        if (complete == 0) {
            ...
            if (cornerbl(0, 0, 0)) {
                l = lengthl();
                if (!cornerblOnl(0, 0, 0) || !cornertlOn(1 + l, 0, 0)
                    || !cornertrOn(1 + l, 0, 1 + l)
                    || !cornerbrOnl(0, 0, 1 + l)) {
                    percentageOn = 0;
                    return false;
                }
                ...
                l = lengthr();
                if (!cornerbrOnr(0, 0, 0) || !cornertrOn(1 + l, 0, 0)
                    || !cornertlOn(1 + l, 0, -(1 + l))
                    || !cornerblOnr(0, 0, -(1 + l))) {
                    percentageOn = 0;
                    return false;
                }
